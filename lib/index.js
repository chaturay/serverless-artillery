...
const temp = BbPromise.promisifyAll(require('temp'))
...
`,
  ...
  tempArtilleryLambda: () => {
    ...
    if (impl.verifyTempArtilleryLambda(userConfig.tempPath)) {
      ...
      return BbPromise.resolve(userConfig.tempPath)
    }

    // Create new temp dir for Artillery Lambda.
    return temp.mkdir('artillery-lambda')
      .then((tempPath) => {
...
module.exports = {
   ...
   * @returns {Promise} A promise that completes after updating the local function assets.
   */
  upgrade: () => {
