...
`,
  ...
  createAcceptanceScript: (script) => {
    ...
    let filename = path.join(os.tmpdir(), `${shortid.generate()}.json`);
    while (fs.existsSync(filename)) {
      filename = path.join(os.tmpdir(), `${shortid.generate()}.json`);
    }
    fs.writeFileSync(filename, JSON.stringify(acceptanceScriptData), { encoding: 'utf8' });
    return filename;
  },
};
