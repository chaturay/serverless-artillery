    ...
    .then((input) => {
      ...
      return impl.serverlessRunner(options).then((result) => {
        ...
        if (options.acceptance || options.monitoring) {
          ...
          const countErrorsInResults = (results) => {
            const messages = Object.getOwnPropertyNames(results.errors)
            return messages.reduce((total, errorMessage) => results.errors[errorMessage] + total, 0)
          }

          const errorCount = countErrorsInResults(result)
