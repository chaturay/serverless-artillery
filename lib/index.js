...
`,
  ...
  /**
   * create a copy of the script that is to be sent to the remote function. Edit the script such that the mode
   * is specified, create a new file for the new script and return the path to the new file.
   * @param script
   */
  createAcceptanceScript: (script) => {
    const acceptanceScriptData = JSON.parse(JSON.stringify(script));
    acceptanceScriptData.mode = 'acc';
    const acceptanceFilePath = path.join(process.cwd(), 'acceptance.json');
    const writefile = JSON.stringify(acceptanceScriptData);
    fs.writeFileSync(acceptanceFilePath, writefile, { encoding: 'utf8' });
...
module.exports = {
   ...
   */
  invoke: (options) => {
    console.log('invoking with this version of serverless-artillery');
    let completeMessage = `${os.EOL}\tYour function invocation has completed.${os.EOL}`;
    let scriptPath = impl.findScriptPath(options.script);
    if (!scriptPath) {
      return BbPromise.reject(new Error(`${os.EOL}\tScript '${options.script}' could not be found.${os.EOL}`));
    }
    // readScript returns json object
    let scriptData = impl.readScript(scriptPath);
    // if in acceptance mode, create new copy of script, add acceptance mode, write as a new file and adjust scriptPath
    if (options.acceptance) {
      scriptPath = impl.createAcceptanceScript(scriptData);
      scriptData = impl.readScript(scriptPath);
    }
    ...
    if (!options.acceptance) {
      ...
      const exceedsLimits = scriptExtent.width >= scriptExtent.maxWidth || scriptExtent.length >= scriptExtent.maxLength; //eslint-disable-line
