...
`,
  ...
  addAssets: (existing, options) => {
    ...
    const publishPolicy = {
      Effect: 'Allow',
      Action: ['sns:Publish'],
      Resource: `!Ref ${constants.AlertingName}${constants.yamlComments.mustMatchKey}`,
    }
    const environment = {
      TOPIC_ARN: `!Ref ${constants.AlertingName}`,
      TOPIC_NAME: `!GetAtt ${constants.AlertingName}.TopicName`,
    }
  ...
  writeBackup: (content, index) => {
    ...
    const target = `${constants.ServerlessFiles[0]}${index ? `.${index}` : ''}.bak`
...
module.exports = {
  ...
  monitor: (options) => {
    ...
    let preRead
    let preWrite
    if (!impl.fileExists(constants.ServerlessFiles[0])) {
      console.log(`${os.EOL}A '${constants.ServerlessFiles[0]}' file not found in the current directory, running configure to generate function assets${os.EOL}`)
      preRead = module.exports.configure(options)
        .then(() => console.log('Function assets generated locally... modifying them for monitoring'))
      preWrite = () => BbPromise.resolve()
    } else {
      preRead = BbPromise.resolve()
        .then(() => console.log(`${os.EOL}Reading your 'serverless.yml' to add monitoring related assets and configuration`))
      preWrite = impl.writeBackup
    }
    return preRead
      .then(() => fs.readFileAsync(constants.ServerlessFiles[0], 'utf8'))
      .then((input) => {
        ...
        return preWrite(input)
          .then(() => fs.writeFileAsync(constants.ServerlessFiles[0], output))
          .then(() => console.log([
            `${os.EOL}Monitoring assets added to serverless.yml${os.EOL}`,
            'Tasks for you:',
            '1. Modify your \'serverless.yml\' to enable subscriptions to any alerts raised by the monitoring function.',
            '   (To help you out, we\'ve provided commented out examples of these for you in \'serverless.yml\')',
            '   (What good is monitoring if noone is listening?)',
            `2. Deploy your new assets/updated service using 'slsart deploy'${os.EOL}`,
            'Thank you for trying monitoring mode, may it help keep your systems up!',
          ].join(os.EOL)))
      })
  },
}
