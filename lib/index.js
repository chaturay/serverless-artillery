...
`,
    ...
    return serverless.init()
      .then(() => { // add our intercepter
        const invoke = serverless.pluginManager.plugins.find(
          plugin => Object.getPrototypeOf(plugin).constructor.name === 'AwsInvoke'
        );
   ...
   * @param script the original script to be copied.
   */
  createAcceptanceScript: (script) => {
    let filepath;
    script.mode = 'acc'; // eslint-disable-line no-param-reassign
    do {
      filepath = path.join(os.tmpdir(), `${shortid.generate()}.json`);
    } while (fs.existsSync(filepath));
    fs.writeFileSync(filepath, JSON.stringify(script), { encoding: 'utf8' });
    return filepath;
  },
};
...
module.exports = {
  ...
  invoke: (options) => {
    ...
    // create new script if in acceptance mode
    ...
    // add optional region argument // TODO better.  we should adopt a total pass-through approach
    if (options.region) {
      process.argv.push('-r');
      process.argv.push(options.region);
    }
    // load and analyze script if tool or script is in performance mode
    if (
      !options.acceptance &&            // not in acceptance mode by CLI flag
      (
        !scriptData.mode ||             // script doesn't define a mode
        (
          scriptData.mode !== 'acc' &&  // script mode doesn't specify acceptance mode
          scriptData.mode !== 'acceptance'
        )
      )
    ) {
