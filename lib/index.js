...
module.exports = {
   ...
   */
  run: (options) => {
    let completeMessage = `${os.EOL}\tTest Lambda complete.${os.EOL}`;
    ...
    if (exceedsLimits) { // exceeds limits?
      ...
      completeMessage = `${os.EOL}\tTest Lambda started. Test will be complete after ${
  ...
  configure: options => new BbPromise((resolve, reject) => {
    ...
    if (conflicts.length) {
      ...
      console.log([
        ...
        `${os.EOL}\tEdit serverless.yml to customize your load function but please note that you must re-deploy the`,
        `${os.EOL}\t\tfunction after making any modifications.`,
