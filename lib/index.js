...
const func = require('./faas/aws-func')
const task = require('./faas/task-artillery')
...
const constants = {
  ...
  ServerlessDirectories: [
    ...
    'aws-func', // !!! This is depended upon in `configure` below, `~/postinstall.js`, and `~/tests/lib/npm.spec.js` !!!
    'task-artillery',
    'serverless-star',
    'aws',
  ],
...
const impl = {
  ...
  findScriptPath: (scriptPath) => {
    if (scriptPath) {
      ...
      if (impl.fileExists(localDefaultScript)) {
        ...
        return path.join(__dirname, 'faas', 'aws', constants.DefaultScriptName)
      }
    }
  },
  /**
  ...
  parseScript: (input) => {
    ...
    func.valid(script)
    const settings = func.define.getSettings(script)
    settings.task = task.define.getSettings(script)
...
`,
  ...
  findServicePath: () => {
    ...
    if (impl.fileExists(localServerlessPath)) {
      ...
      return path.join(__dirname, 'faas', 'aws')
    }
  },
  /**
...
module.exports = {
  ...
  invoke: options => impl.getScriptText(options)
    ...
    .then((input) => {
      ...
      if (!task.define.isSamplingScript(script)) {
      ...
      // run the given script on the deployed function
      logIf(`${os.EOL}\tInvoking test function${os.EOL}`)
    ...
    .catch((ex) => {
      if (ex instanceof func.define.FunctionError) {
        console.error('Error validating function settings:')
      } else if (ex instanceof task.define.TaskError) {
  ...
  /**
   * Kill an invoked function that is actively executing.
   */
  kill: (options) => {
   ...
   */
  script: (options) => {
    const destPath = options.out || constants.DefaultScriptName
  ...
  configure: options => new BbPromise((resolve, reject) => {
    ...
    if (conflicts.length) {
      ...
      constants.ServerlessFiles.forEach((file) => {
        const sourcePath = path.join(__dirname, 'faas', file)
        const destPath = path.join(cwd, file)
      ...
      try {
        constants.ServerlessDirectories.forEach(dir =>
          npm.install(options, path.join(cwd, dir)))
        resolve()
      } catch (ex) {
