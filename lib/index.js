...
const taskPlan = require('./lambda/taskPlan')
...
const impl = {
  ...
  parseInput: (input) => {
    ...
    handler.impl.validScript(script, null, (err) => {
      if (err) {
        throw new Error(`Script validation error: ${err}`)
      }
    })
    return script
  },
  ...
  scriptConstraints(script) {
    ...
    const durationInSeconds = taskPlan.impl.scriptDurationInSeconds(script)
    const requestsPerSecond = taskPlan.impl.scriptRequestsPerSecond(script)
