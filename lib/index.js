...
`,
   ...
   */
  validateServiceForUpgrade: (config, constants, PreconditionError = UpdatePreconditionError, ConflictError = UpdateConflictError) => {
  ...
  update: (
    ...
    const slsYmlPreconditions = () => {
      ...
      if (slsYmlMissing) {
        ...
        return readYml(constants.ServerlessFile)
          .then(yml => impl.validateServiceForUpgrade(yml, constants))
      }
    }
    ...
    return BbPromise.all(preRead)
      ...
      .then((input) => {
        ...
        impl.validateServiceForUpgrade(service)
