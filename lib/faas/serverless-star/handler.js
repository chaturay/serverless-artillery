...
const alert = require('aws-alert')
const func = require('aws-func')
const task = require('task-artillery')
...
const impl = {
   ...
   * @param event The event containing the remaining jobs that is to be sent to the next function
   * @param invocationType The function invocationType
   * @returns {Promise<any>}
   */
  invokeSelf(timeDelay, event, invocationType) {
    ...
    return impl.delay(timeDelay).then(invoke)
  },
  /**
  ...
  execute: (timeNow, sourceEvent, settings, event) => {
    ...
    return impl.delay(timeDelay)
      .then(() => {
        ...
        return task.execute(timeNow, script)
          .then((result) => {
            if (script._trace) {
              console.log(`execution complete from ${script._genesis} in ${timeNow} @ ${Date.now()}`)
            }
            return Promise.resolve(task.result(timeNow, sourceEvent, settings, [result]))
          })
      })
      .catch((ex) => {
