...
const impl = (
  ...
  const inst = {
      ...
      fs.readdirSync('.')
        .forEach(filename =>
          fs.createReadStream(filename)
            .pipe(fs.createWriteStream(path.join(backupDir, filename)))
        )
    },
    ...
    loadUpgradePlugin: (version) => {
      ...
      console.error('2 >>>>', upgradeModule.version)
    ...
    loadPluginForCurrentVersion: () => {
      ...
      console.error('3 >>>', currentVersionPlugin.version)
    ...
    upgradeService: () => {
      ...
      console.error('>>>>>', currentVersionPlugin.version)
      ...
      while (aValidNextVersionExists()) {
        ...
        console.error('4 >>>', nextVersionPlugin.version)
