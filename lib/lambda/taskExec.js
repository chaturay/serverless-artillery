...
const impl = run => ({
  ...
  execLoad: (timeNow, script) => {
    ...
    return new Promise((resolve, reject) => {
      try {
        fs.writeFile(scriptPath, JSON.stringify(script), {}, (writeError) => {
          if (writeError) throw new Error(`Failed to write script file ${scriptPath}: ${writeError}`)
          ...
          process.exit = (code) => {
            ...
            if (code === 0) {
              ...
              fs.readFile(outputPath, null, (readError, results) => {
                if (readError) throw new Error(`Failed to read output file ${outputPath}: ${readError}`)
                resolve(JSON.parse(results))
              })
            } else {
              throw new Error(`Artillery exited with non-zero code: ${code}`)
            }
          }
