...
const impl = {
  ...
  splitScriptByFlow: (script) => {
    ...
    for (i = 0; i < script.scenarios.length; i++) {
      if (script.scenarios[i].flow) {
        ...
        newScript.scenarios = script.scenarios.slice(last, i + 1);
        last = i + 1;
  ...
  runAcceptance: (timeNow, script, context, callback) => {
    ...
    const complete = (data) => {
      ...
      toComplete -= 1;
      if (passed === false) {
        reports.push('Flow failed! See report below for details');
      }
      reports.push(report);
      if (toComplete > 0) { // do we have more outstanding asynchronous operations/callbacks to execute?
        if (script._trace) {
        ...
        callback(null, { message: 'acceptance test from ' + script._genesis + ' completed from ' +
        timeNow + ' @ ' + Date.now(), reports }); // eslint-disable-line object-property-newline
        if (script._trace) {
