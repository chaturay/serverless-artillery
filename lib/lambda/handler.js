...
const impl = {
  ...
  validScript: (script, context, callback) => {
    ...
    } else if (script.mode && !(script.mode === constants.modes.PERF || script.mode === constants.modes.PERFORMANCE
      || script.mode === constants.modes.ACCEPTANCE || script.mode === constants.modes.ACC)) {
      callback(`Artillery script has invalid mode. Valid modes include ${constants.modes.ACCEPTANCE}, 
      ${constants.modes.ACC}, ${constants.modes.PERFORMANCE}, ${constants.modes.PERF}`);
    } else {
  ...
  /**
   * For each flow, create a new script with the same configuration as the original and the single flow. Since
   * this is only used in acceptance mode, the duration and arrivalRate of each new script are set to 1.
   * One phase per script regardless of what was passed in. Flow names and weights are stripped.
   * @param script the script to split on its flows
   * @returns {Array} an array of scripts, each with a single flow from the original script
   */
  splitScriptByFlow: (script) => {
    ...
    // remove all phases but the first one
    if (script.config.phases.length > 1) {
      script.config.phases.length = 1; // eslint-disable-line no-param-reassign
    }
    // change phase values to desired values for acceptance script
    script.config.phases[0].duration = 1; // eslint-disable-line no-param-reassign
    script.config.phases[0].arrivalRate = 1; // eslint-disable-line no-param-reassign
    if (script.config.phases[0].rampTo) {
      delete script.config.phases[0].rampTo; // eslint-disable-line no-param-reassign
    }
    // create an array of scripts, 1 for each flow. scenarios array can contain weights as well, so check for a flow and
    // record where flows exist within the scenarios array
    // TODO allow flow name to be preserved
    for (i = 0; i < script.scenarios.length; i++) {
      if (script.scenarios[i].flow) {
        flowSpots.push(i);
        scripts.push(JSON.parse(JSON.stringify(script)));
      }
    }
    // overwrite the scenarios arrays within the scripts so that each script has a scenario array with a single flow
    for (i = 0; i < flowSpots.length; i++) {
      scripts[i].scenarios[0] = script.scenarios[flowSpots[i]];
      scripts[i].scenarios.length = 1;
    }
    return scripts;
  },
  /**
  ...
  invokeSelf(timeDelay, event, context, callback, invocationType) {
    ...
    function exec() {
      if (event._simulation) {
        ...
        if (event.mode === 'acceptance' || event.mode === 'acc') {
          impl.runAcceptance(Date.now(), event, simulation.context, simulation.callback);
        } else {
          impl.runPerformance(Date.now(), event, simulation.context, simulation.callback);
        }
      }
      // TODO needs to support ._simulation code above
      if (event.mode === 'acceptance' || event.mode === 'acc') {
        lambda.invoke(params, (err, data) => {
          if (err) {
            ...
            if (Object.keys(payload.errors).length) {
              passed = false;
              message = `Error(s) in ${event._start}: ${JSON.stringify(payload.errors)}`;
            } else {
              passed = true;
              message = 'success';
            }
  ...
  /**
   * Split the script on its flows and generate new scripts with one flow each.
   * New flows have an arrival rate of 1 and a duration of 1.
   * Invoke a new lambda for each script and report results
   * TODO: check if an acceptance has been passed in before splitScriptByFlow
   *
   * @param timeNow
   * @param script
   * @param context
  ...
  runAcceptance: (timeNow, script, context, callback) => {
    ...
    let invocationType;
    const responses = [];
    // get number of flows and store in toComplete
    toComplete = 0;
    for (i = 0; i < script.scenarios.length; i++) {
      if (script.scenarios[i].flow) {
        toComplete += 1;
    ...
    console.log(`toComplete: ${toComplete} in ${timeNow} (init)`);
    // complete receives object with data regarding script status and compiles data from each script into an array
    const complete = (data) => {
      if (data.id === ' ') {
        data.id = toComplete; // eslint-disable-line no-param-reassign
      }
      console.log('data');
      console.log(data);
      toComplete -= 1;
      ...
      if (toComplete > 0) { // do we have more outstanding asynchronous operations/callbacks to execute?
        if (script._trace) {
          console.log(`acceptance test from ${script._genesis} executed by ${timeNow} 
            partially complete @ ${Date.now()}`);
        }
      } else { // otherwise, time to complete the lambda
        // eslint-disable-next-line prefer-template
        callback(null, { message: 'acceptance test from ' + script._genesis + ' successfully completed from' + timeNow +
        ' @ ' + Date.now(), responses }); // eslint-disable-line object-property-newline
        if (script._trace) {
    ...
    // if script passed in is already in acceptance test format, run script in lambda
    if (toComplete === 1) {
      if (!script._start) {
        script._start = timeNow; // eslint-disable-line no-param-reassign
      }
    ...
    } else if (toComplete > 1) { // otherwise break script into smaller scripts w one flow each
      scripts = impl.splitScriptByFlow(script, toComplete);
      for (i = 0; i < scripts.length; i++) {
        if (!scripts[i]._start) {
          scripts[i]._start = timeNow; // eslint-disable-line no-param-reassign
        }
        invocationType = 'RequestResponse';
        impl.invokeSelf(scripts[i]._start - Date.now(), scripts[i], context, complete, invocationType);
      }
    }
  },
};
const api = {
  ...
  run: (script, context, callback) => {
    if (impl.validScript(script, context, callback)) {
      const mode = script.mode || constants.modes.PERF;
      const now = Date.now();
      if (!script._genesis) {
        script._genesis = now; // eslint-disable-line no-param-reassign
      }
      if (mode === constants.modes.ACC || mode === constants.modes.ACCEPTANCE) {
