...
const impl = {
  ...
  validScript: (script, context, callback) => {
    ...
    if (script._split && typeof script._split !== 'object') {
      ...
      // Validate the Phases TODO change other functions to check for phase rather than assume it exists (for acc mode)
  ...
  splitScriptByFlow: (script) => {
    ...
    // break each flow into a new script
    let last = 0;
    ...
    const exec = () => {
      try {
        if (event._simulation) {
          ...
          if (event.mode === constants.modes.ACCEPTANCE || event.mode === constants.modes.ACC) {
