...
try {
  const ntpClient = require('ntp-client'); // eslint-disable-line global-require

  ntpClient.getNetworkTime('0.amazon.pool.ntp.org', 123, (err, date) => {
    if (err) {
      console.error(err);
    } else {
      const ntp = date.getTime();
      const now = Date.now();
      const delta = ntp - now;
      if (constants.CLOCK_DRIFT_THRESHOLD < delta) {
        console.log('############################');
        console.log(`!!@@## WARNING - threshold for clock drift (${
          constants.CLOCK_DRIFT_THRESHOLD
