...
const impl = {
   ...
   * One phase per script regardless of what was passed in.
  ...
  splitScriptByFlow: (script) => {
    let i;
    const scripts = [];
    script.config.phases = [ // eslint-disable-line no-param-reassign
      {
        duration: 1,
        arrivalRate: 1,
      },
  ...
  /**
   * Split the script on its flows, generating a new script per flow.  Each script will have an arrival rate of 1 and a
   * duration of 1 so that each flow is executed exactly once by invoking a new lambda to execute it and report results.
   * @param timeNow The time at which the event was received for this execution
   * @param script The Artillery (http://artillery.io) script to execute after optional splitting
   * @param context The Lambda provided execution context
   * @param callback The Lambda provided callback to report errors or success to
   */
  runAcceptance: (timeNow, script, context, callback) => {
    ...
    const complete = (data) => {
      ...
      if (toComplete > 0) { // do we have more outstanding asynchronous operations/callbacks to execute?
        ...
        callback(null, { message: `acceptance test from ${script._genesis} completed from ${timeNow} @ ${Date.now()}` +
          ` with ${failedFlows} failed flow(s)`, reports }); // eslint-disable-line object-property-newline
        if (script._trace) {
    ...
    if (scripts.length === 1) {
      ...
      for (i = 0; i < scripts.length; i++) {
        impl.invokeSelf(scripts[i]._start - Date.now(), scripts[i], context, complete, 'RequestResponse');
      }
    }
  },
};
  ...
  run: (script, context, callback) => {
    if (impl.validScript(script, context, callback)) {
      ...
      if (script.mode === constants.modes.ACC || script.mode === constants.modes.ACCEPTANCE) {
        script._start = now; // eslint-disable-line no-param-reassign
