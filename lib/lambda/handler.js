...
const impl = {
        ...
        runner.on('done', (report) => {
          ...
          if (script.mode === constants.modes.ACC || script.mode === constants.modes.ACCEPTANCE) {
            if (report.errors.length) {
              callback(null, {
                reportMessage: '1 flow failed',
                report,
              });
            } else {
              callback(null, {
                reportMessage: '0 flows failed',
                report,
              });
            }
          } else {
  ...
  runAcceptance: (timeNow, script, context, callback) => {
    ...
    const complete = (data) => {
      ...
      //   failedFlows += 1;
      // }
      if (report.reportMessage === '1 flow failed') {
        failedFlows += 1;
      }
      // only return one report message
      delete (report.reportMessage);
      reports.push(report);
      if (toComplete > 0) { // do we have more outstanding asynchronous operations/callbacks to execute?
        if (script._trace) {
        ...
        if (failedFlows === 1) {
          ...
          reportMessage = `${failedFlows} flows failed`;
        }
        callback(null, {
          reportMessage,
          reports,
