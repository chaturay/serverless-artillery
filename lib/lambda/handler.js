...
const impl = {
  ...
  splitScriptByFlow: (script) => {
    ...
    // iterate through the original scripts scenarios array. each time a flow is found create a copy of the scripts
    // slice the flow out of the original scenarios array, and make the sliced out flow the content of the new scripts
    // scenarios
    ...
    for (i = 0; i < script.scenarios.length; i++) {
      if (script.scenarios[i].flow) {
        ...
        const flow = script.scenarios.slice(i, i + 1);
        newScript.scenarios.length = 1;
        newScript.scenarios = flow;
  ...
  runAcceptance: (timeNow, script, context, callback) => {
    ...
    // TODO just push the payloads to responses array instead of creating a smaller object?
    const complete = (data) => {
      let passed;
      let message;
      ...
      if (Object.keys(payload.errors).length) {
        passed = false;
        message = `Error(s) in ${toComplete}: ${JSON.stringify(payload.errors)}`;
      } else {
        passed = true;
        message = 'success';
      }
      ...
      const ret = {
        id: Object.keys(payload.scenarioCounts) || toComplete, // only if flow name is preserved
        pass: passed,
        msg: message,
      };
      toComplete -= 1;
