...
const impl = {
  ...
  validScript: (script, context, callback) => {
    ...
    // } else if ('mode' in script && !Object.keys(constants.modes).map(function (key) { console.log(constants.modes[key]); return constants.modes[key]; })) {
    //   let message = null;
    //   Object.values(constants.modes).foreach((value) => {
    //     if (message === null) {
    //       message = `If specified, the mode attribute must be one of "${value}"`;
    //     } else {
    //       message += `, "${value}"`;
    //     }
    //   });
  ...
  splitScriptByFlow: (script) => {
    ...
    phase.push(acceptancePhase);
    script.config.phases = phase;// eslint-disable-line no-param-reassign
    console.log('after');
    console.log(script.config);
    // // remove all phases but the first one
    // if (script.config.phases.length > 1) {
    //   script.config.phases.length = 1;
    // }
  ...
  runAcceptance: (timeNow, script, context, callback) => {
    ...
    const complete = (data) => {
      ...
      if (toComplete > 0) { // do we have more outstanding asynchronous operations/callbacks to execute?
        ...
        callback(null, { message: 'acceptance test from ' + script._genesis + ' successfully completed from ' +
        timeNow + ' @ ' + Date.now(), responses }); // eslint-disable-line object-property-newline
        if (script._trace) {
    ...
    // if script passed in is already in acceptance test format, run script in current lambda
    // TODO check the duration and arrivalRate
    if (toComplete === 1) {
      if (!script._start) {
        script._start = timeNow; // eslint-disable-line no-param-reassign
      }
      ...
      scripts = impl.splitScriptByFlow(script);
