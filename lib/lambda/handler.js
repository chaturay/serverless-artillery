...
let artillery = require('artillery-core'),
    ...
    impl = {
        ...
        readPayload(script) {
            let ret,
                determinePayloadPath = (payloadFile) => {
                    return path.resolve(process.cwd(), payloadFile);
                },
                readSinglePayload = (payloadObject) => {
                    let payloadPath = determinePayloadPath(payloadObject.path);
                    let data = fs.readFileSync(payloadPath, 'utf-8');
                    return csv(data);
                };
