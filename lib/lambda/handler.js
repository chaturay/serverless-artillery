...
const taskExec = require('./taskExec')
...
const impl = {
  ...
  validScript: (script, context, callback) => {
    ...
    if ('_split' in script && typeof script._split !== 'object') {
      ...
      scriptDurationInSeconds = taskPlan.impl.scriptDurationInSeconds(script)
      scriptRequestsPerSecond = taskPlan.impl.scriptRequestsPerSecond(script)
  ...
  runPerformance: (timeNow, script, context, callback) => {
    const settings = impl.getSettings(script)
    const plans = taskPlan(timeNow, script, settings)
    if (plans.length > 1) {
      ...
      impl.delay(timeDelay)
        .then(() => {
          ...
          taskExec(timeNow, script, context, callback)
        })
        .catch((ex) => {
