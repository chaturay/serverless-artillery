...
const impl = {
  ...
  getErrorBudget: (script, defaultErrorBudget) => {
    if (script && script.sampling && script.sampling.errorBudget) {
      ...
      return def.sampling.DefaultErrorBudget
    }
  },
  ...
  analyzeSamples: (script, reports, defaultErrorBudget) => {
    ...
    const sumErrors = subreport =>
      Object.keys(subreport.errors)
        .reduce((total, errorMessage) => console.log(errorMessage, total, subreport.errors[errorMessage]) || (total + subreport.errors[errorMessage]), 0)
    const report = {
      errors: 0,
      reports,
    }
    reports.forEach((subreport) => {
      const errorCount = sumErrors(subreport)
      if (
        subreport &&
        subreport.errors &&
