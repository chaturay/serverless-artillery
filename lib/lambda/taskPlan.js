...
const impl = {
  ...
  planPerformance: (timeNow, event, settings) => {
    ...
    if (scriptDurationInSeconds > settings.maxChunkDurationInSeconds) {
      const parts = impl.splitScriptByDurationInSecondsAndSchedule(script, settings)
      script = parts.chunk
  ...
  /**
   * Plan for exexcuting an acceptance script, executing each of the given script's flows exactly once and generating a
   * report of the success or failure of these acceptance tests.
  ...
  planAcceptance: (timeNow, event) => {
    ...
    return impl.splitScriptByFlow(script)
  },
  /**
  ...
  planTask: (timeNow, script, settings) => {
    if (script.mode === def.modes.ACC || script.mode === def.modes.ACCEPTANCE) {
      return impl.planAcceptance(timeNow, script)
    } else {
